...
* 因為 jQuery 1.6 中已不再支援 form.elements，所以請勿使用。 無論如何還是有很多設計錯誤 (IE6-8: form.elements === form)。
...
* 已修正信用卡驗證，只接受數字及連字號 ("asdf" 不是有效的信用卡號碼)
...
* 已修正選用方法，當欄位為空白時，完全不反白顯示任何項目，也就是不會觸發「成功」
...
* 已完全移除項目識別碼上的相依性，雖然仍會將其用於將錯誤標籤連結至輸入 (若有出現)。 已藉由針對內部 errorList 搭配 {name, message, element} 使用陣列，而不是具有 id:message 配對的物件來達成。
...
* 已簡化 API 來支援不同的事件。 您可以停用預設值 submit。 若指定了任何事件，即會將其套用至每個項目 (而非整個表單)。 將 Keyup 驗證與提交驗證相結合，現在非常容易設定
