...
* [HasChanged](/dotnet/api/microsoft.extensions.primitives.ichangetoken.haschanged):此屬性，可進行檢查以判斷是否已發生變更。
* [RegisterChangeCallback](/dotnet/api/microsoft.extensions.primitives.ichangetoken.registerchangecallback):當偵測到指定的路徑字串變更時呼叫。 每個變更權杖都只會呼叫其相關聯的回呼，以回應單一變更。 若要啟用持續監視，請使用 [TaskCompletionSource](/dotnet/api/system.threading.tasks.taskcompletionsource-1) (如下所示) 或重新建立 `IChangeToken` 執行個體以回應變更。
