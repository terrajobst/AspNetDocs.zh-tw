...
## <a name="the-top-8-programming-tips"></a><span data-ttu-id="b54d3-116">前8個程式設計提示</span><span class="sxs-lookup"><span data-stu-id="b54d3-116">The Top 8 Programming Tips</span></span>
...
> 
> <span data-ttu-id="b54d3-131">您可以閱讀更多有關[使用表單](https://go.microsoft.com/fwlink/?LinkId=202892)的 HTML 編碼方式。</span><span class="sxs-lookup"><span data-stu-id="b54d3-131">You can read more about HTML encoding in [Working with Forms](https://go.microsoft.com/fwlink/?LinkId=202892).</span></span>
...
2. <span data-ttu-id="b54d3-185">將下列程式碼和標記複製到頁面中，並取代頁面中已存在的任何專案。</span><span class="sxs-lookup"><span data-stu-id="b54d3-185">Copy the following code and markup into the page, replacing anything already in the page.</span></span>  
    ...
    - <span data-ttu-id="b54d3-187">`@` 字元會啟動頁面中第一個程式碼區塊，並在靠近頁面底部的 `totalMessage` 變數之前。</span><span class="sxs-lookup"><span data-stu-id="b54d3-187">The `@` character starts the first block of code in the page, and it precedes the `totalMessage` variable that's embedded near the bottom of the page.</span></span>
...
<span data-ttu-id="b54d3-205">但首先，有點的技術背景。</span><span class="sxs-lookup"><span data-stu-id="b54d3-205">But first, a little technical background.</span></span>
...
<span data-ttu-id="b54d3-300">您可以使用 `var` 關鍵字（如果您不想指定類型）或使用類型的名稱來宣告變數：</span><span class="sxs-lookup"><span data-stu-id="b54d3-300">You declare a variable using the `var` keyword (if you don't want to specify a type) or by using the name of the type:</span></span>
...
<span data-ttu-id="b54d3-317">下表列出一些常見的變數轉換和測試方法。</span><span class="sxs-lookup"><span data-stu-id="b54d3-317">The following table lists some common conversion and test methods for variables.</span></span>
...
<span data-ttu-id="b54d3-329">「運算子」是一個關鍵字或字元，告訴 ASP.NET 要在運算式中執行哪一種命令。</span><span class="sxs-lookup"><span data-stu-id="b54d3-329">An operator is a keyword or character that tells ASP.NET what kind of command to perform in an expression.</span></span> <span data-ttu-id="b54d3-330">C#語言（以及以它為基礎的 Razor 語法）支援許多運算子，但您只需要辨識幾個，即可開始使用。</span><span class="sxs-lookup"><span data-stu-id="b54d3-330">The C# language (and the Razor syntax that's based on it) supports many operators, but you only need to recognize a few to get started.</span></span> <span data-ttu-id="b54d3-331">下表摘要說明最常見的運算子。</span><span class="sxs-lookup"><span data-stu-id="b54d3-331">The following table summarizes the most common operators.</span></span>
...
### <a name="testing-conditions"></a><span data-ttu-id="b54d3-403">測試條件</span><span class="sxs-lookup"><span data-stu-id="b54d3-403">Testing Conditions</span></span>

<span data-ttu-id="b54d3-404">若要測試簡單的條件，請使用 `if` 語句，這會根據您指定的測試傳回 true 或 false：</span><span class="sxs-lookup"><span data-stu-id="b54d3-404">To test a simple condition you use the `if` statement, which returns true or false based on a test you specify:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample49.cshtml)]

<span data-ttu-id="b54d3-405">`if` 關鍵字會啟動區塊。</span><span class="sxs-lookup"><span data-stu-id="b54d3-405">The `if` keyword starts a block.</span></span> <span data-ttu-id="b54d3-406">實際測試（條件）在括弧中，並傳回 true 或 false。</span><span class="sxs-lookup"><span data-stu-id="b54d3-406">The actual test (condition) is in parentheses and returns true or false.</span></span> <span data-ttu-id="b54d3-407">如果測試為 true，則執行的語句會以大括弧括住。</span><span class="sxs-lookup"><span data-stu-id="b54d3-407">The statements that run if the test is true are enclosed in braces.</span></span> <span data-ttu-id="b54d3-408">`if` 語句可以包含 `else` 區塊，指定當條件為 false 時要執行的語句：</span><span class="sxs-lookup"><span data-stu-id="b54d3-408">An `if` statement can include an `else` block that specifies statements to run if the condition is false:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample50.cshtml)]

<span data-ttu-id="b54d3-409">您可以使用 `else if` 區塊來新增多個條件：</span><span class="sxs-lookup"><span data-stu-id="b54d3-409">You can add multiple conditions using an `else if` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample51.cshtml)]

<span data-ttu-id="b54d3-410">在此範例中，如果 if 區塊中的第一個條件不是 true，則會核取 [`else if`] 條件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-410">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="b54d3-411">如果符合該條件，就會執行 `else if` 區塊中的語句。</span><span class="sxs-lookup"><span data-stu-id="b54d3-411">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="b54d3-412">如果沒有符合任何條件，則會執行 `else` 區塊中的語句。</span><span class="sxs-lookup"><span data-stu-id="b54d3-412">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="b54d3-413">您可以新增任意數目的 if 區塊，然後以 `else` 區塊關閉，因為 &quot;所有其他專案&quot; 條件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-413">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="b54d3-414">若要測試大量的條件，請使用 `switch` 組塊：</span><span class="sxs-lookup"><span data-stu-id="b54d3-414">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="b54d3-415">要測試的值是以括弧括住（在範例中為 `weekday` 變數）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-415">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="b54d3-416">每個個別測試都會使用以冒號（:) 結尾的 `case` 語句。</span><span class="sxs-lookup"><span data-stu-id="b54d3-416">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="b54d3-417">如果 `case` 語句的值符合測試值，則會執行該案例區塊中的程式碼。</span><span class="sxs-lookup"><span data-stu-id="b54d3-417">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="b54d3-418">您可以使用 `break` 語句來關閉每個 case 語句。</span><span class="sxs-lookup"><span data-stu-id="b54d3-418">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="b54d3-419">（如果您忘了在每個 `case` 區塊中包含 break，則下一個 `case` 語句的程式碼也會執行）。`switch` 區塊通常會有 `default` 的語句做為 &quot;所有其他情況的&quot; 選項（如果沒有其他情況成立）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-419">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="b54d3-420">在瀏覽器中顯示的最後兩個條件式區塊的結果：</span><span class="sxs-lookup"><span data-stu-id="b54d3-420">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor-Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="b54d3-422">迴圈程式碼</span><span class="sxs-lookup"><span data-stu-id="b54d3-422">Looping Code</span></span>

<span data-ttu-id="b54d3-423">您通常需要重複執行相同的語句。</span><span class="sxs-lookup"><span data-stu-id="b54d3-423">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="b54d3-424">您可以藉由迴圈來完成這項操作。</span><span class="sxs-lookup"><span data-stu-id="b54d3-424">You do this by looping.</span></span> <span data-ttu-id="b54d3-425">例如，您通常會針對資料集合中的每個專案執行相同的語句。</span><span class="sxs-lookup"><span data-stu-id="b54d3-425">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="b54d3-426">如果您確切知道您想要迴圈的次數，可以使用 `for` 迴圈。</span><span class="sxs-lookup"><span data-stu-id="b54d3-426">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="b54d3-427">這種迴圈特別適合用來計算或計數：</span><span class="sxs-lookup"><span data-stu-id="b54d3-427">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="b54d3-428">迴圈會以 `for` 關鍵字開頭，後面接著以括弧括住的三個語句，每個都以分號結束。</span><span class="sxs-lookup"><span data-stu-id="b54d3-428">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="b54d3-429">在括弧內，第一個語句（`var i=10;`）會建立一個計數器，並將它初始化為10個。</span><span class="sxs-lookup"><span data-stu-id="b54d3-429">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="b54d3-430">您不需要將計數器命名 `i` &#8212;您可以使用任何變數。</span><span class="sxs-lookup"><span data-stu-id="b54d3-430">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="b54d3-431">當 `for` 迴圈執行時，計數器會自動遞增。</span><span class="sxs-lookup"><span data-stu-id="b54d3-431">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="b54d3-432">第二個語句（`i < 21;`）會設定您想要計算的距離條件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-432">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="b54d3-433">在此情況下，您想要將它移到最多20個（也就是，在計數器小於21時繼續進行）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-433">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="b54d3-434">第三個語句（`i++`）使用遞增運算子，這只會指定在每次執行迴圈時，計數器應該加入1。</span><span class="sxs-lookup"><span data-stu-id="b54d3-434">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="b54d3-435">括弧內的程式碼會針對迴圈的每個反復專案執行。</span><span class="sxs-lookup"><span data-stu-id="b54d3-435">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="b54d3-436">標記會每次建立新的段落（`<p>` 元素），並在輸出中加入一行，顯示 `i` （計數器）的值。</span><span class="sxs-lookup"><span data-stu-id="b54d3-436">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="b54d3-437">當您執行此頁面時，此範例會建立顯示輸出的11行，而每一行中的文字會指出專案編號。</span><span class="sxs-lookup"><span data-stu-id="b54d3-437">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor-Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="b54d3-439">如果您使用的是集合或陣列，通常會使用 `foreach` 迴圈。</span><span class="sxs-lookup"><span data-stu-id="b54d3-439">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="b54d3-440">集合是一組類似的物件，而 `foreach` 迴圈可讓您在集合中的每個專案上執行工作。</span><span class="sxs-lookup"><span data-stu-id="b54d3-440">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="b54d3-441">這種類型的迴圈對集合很方便，因為與 `for` 迴圈不同的是，您不需要遞增計數器或設定限制。</span><span class="sxs-lookup"><span data-stu-id="b54d3-441">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="b54d3-442">相反地，`foreach` 迴圈程式碼只會繼續完成集合，直到完成為止。</span><span class="sxs-lookup"><span data-stu-id="b54d3-442">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="b54d3-443">例如，下列程式碼會傳回 `Request.ServerVariables` 集合中的專案，這是包含您的 web 伺服器相關資訊的物件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-443">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="b54d3-444">它會使用 `foreac` h 迴圈來顯示每個專案的名稱，方法是在 HTML 項目符號清單中建立新的 `<li>` 元素。</span><span class="sxs-lookup"><span data-stu-id="b54d3-444">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="b54d3-445">`foreach` 關鍵字後面加上括弧，您可以在其中宣告代表集合中單一專案的變數（在範例中，`var item`），後面接著 `in` 關鍵字，後面接著您要迴圈執行的集合。</span><span class="sxs-lookup"><span data-stu-id="b54d3-445">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="b54d3-446">在 `foreach` 迴圈的主體中，您可以使用先前宣告的變數來存取目前的專案。</span><span class="sxs-lookup"><span data-stu-id="b54d3-446">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor-Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="b54d3-448">若要建立更一般用途的迴圈，請使用 `while` 語句：</span><span class="sxs-lookup"><span data-stu-id="b54d3-448">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="b54d3-449">`while` 迴圈是以 `while` 關鍵字開頭，後面接著括弧，您可以在其中指定迴圈繼續的時間（在這裡，只要 `countNum` 小於50），就會重複區塊。</span><span class="sxs-lookup"><span data-stu-id="b54d3-449">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="b54d3-450">迴圈通常會遞增（加入）或遞減（減去）用來計算的變數或物件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-450">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="b54d3-451">在此範例中，`+=` 運算子會在每次執行迴圈時，將1新增至 `countNum`。</span><span class="sxs-lookup"><span data-stu-id="b54d3-451">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="b54d3-452">（若要遞減迴圈中計數的變數，您可以使用遞減運算子 `-=`）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-452">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="b54d3-453">物件和集合</span><span class="sxs-lookup"><span data-stu-id="b54d3-453">Objects and Collections</span></span>

<span data-ttu-id="b54d3-454">ASP.NET 網站中幾乎所有內容都是物件，包括網頁本身。</span><span class="sxs-lookup"><span data-stu-id="b54d3-454">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="b54d3-455">本節討論您在程式碼中經常使用的一些重要物件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-455">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="b54d3-456">頁面物件</span><span class="sxs-lookup"><span data-stu-id="b54d3-456">Page Objects</span></span>

<span data-ttu-id="b54d3-457">ASP.NET 中最基本的物件是頁面。</span><span class="sxs-lookup"><span data-stu-id="b54d3-457">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="b54d3-458">您可以直接存取頁面物件的屬性，而不需要任何限定的物件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-458">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="b54d3-459">下列程式碼會使用頁面的 `Request` 物件，取得頁面的檔案路徑：</span><span class="sxs-lookup"><span data-stu-id="b54d3-459">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="b54d3-460">若要清楚指出您是在目前頁面物件上參考屬性和方法，您可以選擇性地使用關鍵字 `this` 來代表程式碼中的頁面物件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-460">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="b54d3-461">以下是先前的程式碼範例，加上 `this` 以代表該頁面：</span><span class="sxs-lookup"><span data-stu-id="b54d3-461">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="b54d3-462">您可以使用 `Page` 物件的屬性來取得大量資訊，例如：</span><span class="sxs-lookup"><span data-stu-id="b54d3-462">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="b54d3-463">`Request`</span><span class="sxs-lookup"><span data-stu-id="b54d3-463">`Request`.</span></span> <span data-ttu-id="b54d3-464">如您所見，這是目前要求的相關資訊集合，包括提出要求的瀏覽器類型、頁面的 URL、使用者身分識別等。</span><span class="sxs-lookup"><span data-stu-id="b54d3-464">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="b54d3-465">`Response`</span><span class="sxs-lookup"><span data-stu-id="b54d3-465">`Response`.</span></span> <span data-ttu-id="b54d3-466">這是在伺服器程式碼完成執行時，將會傳送至瀏覽器的回應（頁面）相關資訊集合。</span><span class="sxs-lookup"><span data-stu-id="b54d3-466">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="b54d3-467">例如，您可以使用這個屬性將資訊寫入至回應。</span><span class="sxs-lookup"><span data-stu-id="b54d3-467">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="b54d3-468">集合物件（陣列和字典）</span><span class="sxs-lookup"><span data-stu-id="b54d3-468">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="b54d3-469">*集合*是一組相同類型的物件，例如從資料庫 `Customer` 物件的集合。</span><span class="sxs-lookup"><span data-stu-id="b54d3-469">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="b54d3-470">ASP.NET 包含許多內建集合，例如 `Request.Files` 集合。</span><span class="sxs-lookup"><span data-stu-id="b54d3-470">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="b54d3-471">您通常會使用集合中的資料。</span><span class="sxs-lookup"><span data-stu-id="b54d3-471">You'll often work with data in collections.</span></span> <span data-ttu-id="b54d3-472">有兩個常見的集合類型是*陣列*和*字典*。</span><span class="sxs-lookup"><span data-stu-id="b54d3-472">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="b54d3-473">當您想要儲存類似專案的集合，但不想要建立個別的變數來保存每個專案時，陣列會很有用：</span><span class="sxs-lookup"><span data-stu-id="b54d3-473">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="b54d3-474">使用陣列，您可以宣告特定的資料類型，例如 `string`、`int`或 `DateTime`。</span><span class="sxs-lookup"><span data-stu-id="b54d3-474">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="b54d3-475">若要指出變數可以包含陣列，您可以在宣告中加上括弧（例如 `string[]` 或 `int[]`）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-475">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="b54d3-476">您可以使用物件的位置（索引），或使用 `foreach` 語句來存取陣列中的專案。</span><span class="sxs-lookup"><span data-stu-id="b54d3-476">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="b54d3-477">陣列索引以零&#8212;為起始，亦即，第一個專案位於位置0，第二個專案位於位置1，依此類推。</span><span class="sxs-lookup"><span data-stu-id="b54d3-477">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="b54d3-478">您可以藉由取得 `Length` 屬性來判斷陣列中的專案數。</span><span class="sxs-lookup"><span data-stu-id="b54d3-478">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="b54d3-479">若要取得陣列中特定專案的位置（以搜尋陣列），請使用 `Array.IndexOf` 方法。</span><span class="sxs-lookup"><span data-stu-id="b54d3-479">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="b54d3-480">您也可以執行一些動作，像是反轉陣列的內容（`Array.Reverse` 方法）或排序內容（`Array.Sort` 方法）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-480">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="b54d3-481">在瀏覽器中顯示的字串陣列程式碼輸出：</span><span class="sxs-lookup"><span data-stu-id="b54d3-481">The output of the string array code displayed in a browser:</span></span>

![Razor-Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="b54d3-483">字典是索引鍵/值組的集合，您可以在其中提供用來設定或抓取對應值的索引鍵（或名稱）：</span><span class="sxs-lookup"><span data-stu-id="b54d3-483">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="b54d3-484">若要建立字典，您可以使用 `new` 關鍵字來表示您要建立新的字典物件。</span><span class="sxs-lookup"><span data-stu-id="b54d3-484">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="b54d3-485">您可以使用 `var` 關鍵字，將字典指派給變數。</span><span class="sxs-lookup"><span data-stu-id="b54d3-485">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="b54d3-486">您可以使用角括弧（`< >`）來指示字典中專案的資料類型。</span><span class="sxs-lookup"><span data-stu-id="b54d3-486">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="b54d3-487">在宣告的結尾，您必須加入一對括弧，因為這實際上是建立新字典的方法。</span><span class="sxs-lookup"><span data-stu-id="b54d3-487">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="b54d3-488">若要將專案加入至字典，您可以呼叫 dictionary 變數的 `Add` 方法（在此案例中為`myScores`），然後指定索引鍵和值。</span><span class="sxs-lookup"><span data-stu-id="b54d3-488">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="b54d3-489">或者，您可以使用方括弧來表示金鑰，並執行簡單的指派，如下列範例所示：</span><span class="sxs-lookup"><span data-stu-id="b54d3-489">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="b54d3-490">若要從字典取得值，請在括弧中指定索引鍵：</span><span class="sxs-lookup"><span data-stu-id="b54d3-490">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="b54d3-491">使用參數呼叫方法</span><span class="sxs-lookup"><span data-stu-id="b54d3-491">Calling Methods with Parameters</span></span>

<span data-ttu-id="b54d3-492">如您稍早在本文中所閱讀，使用進行程式設計的物件可以有方法。</span><span class="sxs-lookup"><span data-stu-id="b54d3-492">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="b54d3-493">例如，`Database` 物件可能會有 `Database.Connect` 的方法。</span><span class="sxs-lookup"><span data-stu-id="b54d3-493">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="b54d3-494">許多方法也有一或多個參數。</span><span class="sxs-lookup"><span data-stu-id="b54d3-494">Many methods also have one or more parameters.</span></span> <span data-ttu-id="b54d3-495">*參數*是您傳遞給方法的值，可讓方法完成其工作。</span><span class="sxs-lookup"><span data-stu-id="b54d3-495">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="b54d3-496">例如，查看 `Request.MapPath` 方法的宣告，其採用三個參數：</span><span class="sxs-lookup"><span data-stu-id="b54d3-496">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="b54d3-497">（這一行已包裝，使其更容易閱讀。</span><span class="sxs-lookup"><span data-stu-id="b54d3-497">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="b54d3-498">請記住，除了括在引號中的字串之外，您幾乎可以在任何地方放置分行符號）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-498">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="b54d3-499">這個方法會傳回伺服器上對應至指定虛擬路徑的實體路徑。</span><span class="sxs-lookup"><span data-stu-id="b54d3-499">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="b54d3-500">方法的三個參數是 `virtualPath`、`baseVirtualDir`和 `allowCrossAppMapping`。</span><span class="sxs-lookup"><span data-stu-id="b54d3-500">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="b54d3-501">（請注意，在宣告中，參數會列出其接受的資料類型）。當您呼叫這個方法時，您必須提供所有三個參數的值。</span><span class="sxs-lookup"><span data-stu-id="b54d3-501">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="b54d3-502">Razor 語法提供兩個選項，讓您將參數傳遞給方法：*位置參數*和*具名引數*。</span><span class="sxs-lookup"><span data-stu-id="b54d3-502">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="b54d3-503">若要使用位置參數呼叫方法，請以方法宣告中指定的嚴格順序來傳遞參數。</span><span class="sxs-lookup"><span data-stu-id="b54d3-503">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="b54d3-504">（您通常會藉由閱讀方法的檔來得知此順序）。您必須遵循順序，而且如果有必要，就無法略過&#8212;任何參數，您可以為沒有值的位置參數傳遞空字串（`""`）或 `null`。</span><span class="sxs-lookup"><span data-stu-id="b54d3-504">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="b54d3-505">下列範例假設您的網站上有一個名為 [*腳本*] 的資料夾。</span><span class="sxs-lookup"><span data-stu-id="b54d3-505">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="b54d3-506">程式碼會呼叫 `Request.MapPath` 方法，並以正確的順序傳遞三個參數的值。</span><span class="sxs-lookup"><span data-stu-id="b54d3-506">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="b54d3-507">然後，它會顯示所產生的對應路徑。</span><span class="sxs-lookup"><span data-stu-id="b54d3-507">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="b54d3-508">當方法有許多參數時，您可以使用具名引數，讓您的程式碼更容易閱讀。</span><span class="sxs-lookup"><span data-stu-id="b54d3-508">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="b54d3-509">若要使用具名引數來呼叫方法，請指定參數名稱，後面接著冒號（:)，然後是值。</span><span class="sxs-lookup"><span data-stu-id="b54d3-509">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="b54d3-510">具名引數的優點是您可以依照您想要的任何順序傳遞它們。</span><span class="sxs-lookup"><span data-stu-id="b54d3-510">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="b54d3-511">（缺點是方法呼叫不是精簡的）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-511">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="b54d3-512">下列範例會呼叫與上述相同的方法，但會使用具名引數來提供值：</span><span class="sxs-lookup"><span data-stu-id="b54d3-512">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="b54d3-513">如您所見，參數會以不同的順序傳遞。</span><span class="sxs-lookup"><span data-stu-id="b54d3-513">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="b54d3-514">不過，如果您執行上述範例和此範例，則會傳回相同的值。</span><span class="sxs-lookup"><span data-stu-id="b54d3-514">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="b54d3-515">處理錯誤</span><span class="sxs-lookup"><span data-stu-id="b54d3-515">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="b54d3-516">Try-catch 語句</span><span class="sxs-lookup"><span data-stu-id="b54d3-516">Try-Catch Statements</span></span>

<span data-ttu-id="b54d3-517">您的程式碼中通常會有語句，可能會因為控制項以外的原因而失敗。</span><span class="sxs-lookup"><span data-stu-id="b54d3-517">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="b54d3-518">例如:</span><span class="sxs-lookup"><span data-stu-id="b54d3-518">For example:</span></span>

- <span data-ttu-id="b54d3-519">如果您的程式碼嘗試建立或存取檔案，可能會發生各種錯誤。</span><span class="sxs-lookup"><span data-stu-id="b54d3-519">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="b54d3-520">您想要的檔案可能不存在、可能被鎖定、程式碼可能沒有許可權等等。</span><span class="sxs-lookup"><span data-stu-id="b54d3-520">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="b54d3-521">同樣地，如果您的程式碼嘗試更新資料庫中的記錄，可能會有許可權問題，可能會卸載與資料庫的連接、要儲存的資料可能無效等等。</span><span class="sxs-lookup"><span data-stu-id="b54d3-521">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="b54d3-522">在程式設計的詞彙中，這些情況稱為*例外*狀況。</span><span class="sxs-lookup"><span data-stu-id="b54d3-522">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="b54d3-523">如果您的程式碼遇到例外狀況，它會產生（擲回）一則錯誤訊息，其中最棒的是使用者很討厭：</span><span class="sxs-lookup"><span data-stu-id="b54d3-523">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor-Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="b54d3-525">在您的程式碼可能會遇到例外狀況的情況下，為了避免這種類型的錯誤訊息，您可以使用 `try/catch` 語句。</span><span class="sxs-lookup"><span data-stu-id="b54d3-525">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="b54d3-526">在 `try` 語句中，您會執行您要檢查的程式碼。</span><span class="sxs-lookup"><span data-stu-id="b54d3-526">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="b54d3-527">在一或多個 `catch` 語句中，您可以尋找可能發生的特定錯誤（特定類型的例外狀況）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-527">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="b54d3-528">您可以視需要包含多個 `catch` 語句，以尋找您預期的錯誤。</span><span class="sxs-lookup"><span data-stu-id="b54d3-528">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="b54d3-529">我們建議您避免在 `try/catch` 語句中使用 `Response.Redirect` 方法，因為它可能會在您的頁面中造成例外狀況。</span><span class="sxs-lookup"><span data-stu-id="b54d3-529">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>

<span data-ttu-id="b54d3-530">下列範例顯示的頁面會在第一個要求上建立文字檔，然後顯示一個按鈕，讓使用者開啟檔案。</span><span class="sxs-lookup"><span data-stu-id="b54d3-530">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="b54d3-531">此範例刻意使用錯誤的檔案名，因此會造成例外狀況。</span><span class="sxs-lookup"><span data-stu-id="b54d3-531">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="b54d3-532">此程式碼包含兩個可能例外狀況的 `catch` 語句： `FileNotFoundException`，如果檔案名不正確，就會發生這種情況，如果 ASP.NET 找不到資料夾，就會發生 `DirectoryNotFoundException`。</span><span class="sxs-lookup"><span data-stu-id="b54d3-532">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="b54d3-533">（您可以將範例中的語句取消批註，以便在所有專案正常運作時查看其執行方式）。</span><span class="sxs-lookup"><span data-stu-id="b54d3-533">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="b54d3-534">如果您的程式碼未處理例外狀況，您會看到如先前螢幕擷取畫面所示的錯誤頁面。</span><span class="sxs-lookup"><span data-stu-id="b54d3-534">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="b54d3-535">不過，`try/catch` 區段有助於防止使用者看到這些類型的錯誤。</span><span class="sxs-lookup"><span data-stu-id="b54d3-535">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="b54d3-536">其他資源</span><span class="sxs-lookup"><span data-stu-id="b54d3-536">Additional Resources</span></span>

<span data-ttu-id="b54d3-537">**使用 Visual Basic 進行程式設計**</span><span class="sxs-lookup"><span data-stu-id="b54d3-537">**Programming with Visual Basic**</span></span>

[<span data-ttu-id="b54d3-538">附錄： Visual Basic 語言和語法</span><span class="sxs-lookup"><span data-stu-id="b54d3-538">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)

<span data-ttu-id="b54d3-539">**參考文件**</span><span class="sxs-lookup"><span data-stu-id="b54d3-539">**Reference Documentation**</span></span>

[<span data-ttu-id="b54d3-540">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="b54d3-540">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="b54d3-541">C#語言</span><span class="sxs-lookup"><span data-stu-id="b54d3-541">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
