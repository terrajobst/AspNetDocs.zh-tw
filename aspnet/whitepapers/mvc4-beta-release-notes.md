...
- **現代 HTTP 程式設計模型：** 直接存取，和處理 HTTP 要求和回應中使用新的強型別 HTTP 物件模型對 Web Api。 相同程式設計模型和 HTTP 管線功能數採對稱式用戶端透過新的 HttpClient 型別。
...
- **查詢撰寫：** 只要傳回 IQueryable&lt;T&gt;，您的 Web API 會支援查詢透過 OData URL 慣例。
- **可測試性改善的 HTTP 詳細資料：** 而不是靜態內容物件中設定 HTTP 詳細資料，現在可以搭配 HttpRequestMessage 和 HttpResponseMessage 的執行個體的 Web API 動作。 可讓您使用您的自訂類型，除了 HTTP 型別也有這些物件的泛型版本。
- **改善的反向的控制 (IoC) 透過 dependencyresolver 來註冊：** Web API 現在會使用 MVC 的相依性解析程式所實作的服務定位器模式來取得執行個體的許多不同的功能。
- **程式碼為基礎的組態：** Web API 組態是只會透過程式碼，離開您的組態檔的初始狀態來完成。
