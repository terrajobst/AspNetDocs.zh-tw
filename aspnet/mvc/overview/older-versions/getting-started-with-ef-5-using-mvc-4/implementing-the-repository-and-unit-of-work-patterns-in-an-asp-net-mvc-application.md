...
在本教學課程系列中，您將不會建立單元測試。 使用儲存機制模式的 MVC 應用程式使用 tdd 的簡介，請參閱[逐步解說：使用 TDD 搭配 ASP.NET MVC](https://msdn.microsoft.com/library/ff847525.aspx)。 如需儲存機制模式的詳細資訊，請參閱下列資源：
...
在  *StudentController.cs*，目前在類別中的程式碼取代為下列程式碼。 所做的變更已醒目標示。
