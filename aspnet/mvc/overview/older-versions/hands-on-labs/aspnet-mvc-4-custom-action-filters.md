...
此 「 音樂市集 」 解決方案有新的資料模型資料表進行網站記錄**ActionLog**，具有下列欄位：接收要求、 呼叫動作、 用戶端 IP 和時間戳記的控制站的名稱。
...
> 
> - **OnActionExecuting**(ActionExecutingContext filterContext):之前稱為動作方法。
> - **OnActionExecuted**(ActionExecutedContext filterContext):在呼叫動作方法之後和之前的結果執行 （在之前檢視呈現）。
