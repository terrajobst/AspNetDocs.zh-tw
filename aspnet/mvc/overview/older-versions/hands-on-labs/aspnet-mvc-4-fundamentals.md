...
7. [練習 7:認識 ASP.NET MVC 4 的新範本](#Exercise7)
...
> [!NOTE]
> 何謂**清單&lt;專輯&gt;** ？:使用此定義**清單&lt;T&gt;** 類型，其中**T**限制哪些項目，這個型別**清單**屬於，在此情況下**專輯**（或其任何子系）。
> 
...
3. 在 **加入檢視**對話方塊方塊中，確認檢視名稱**瀏覽**。 請檢查**建立強型別檢視**核取方塊，然後選取**StoreBrowseViewModel**從**模型類別**下拉式清單。 其他欄位保留其預設值。 然後按一下 [加入]  。
    ...
    *加入瀏覽檢視*
4. 修改**Browse.cshtml**要顯示的內容類型的詳細資訊，存取**StoreBrowseViewModel**傳遞至檢視範本的物件。 若要這樣做，請以下列取代內容：(C#)
...
2. 在 [**加入檢視**] 對話方塊中，確認**檢視名稱**是**詳細資料**。 請檢查**建立強型別檢視**核取方塊，然後選取**專輯**從**模型類別**下拉式清單。 其他欄位保留其預設值。 然後按一下 [加入]  。 這會建立並開啟 **\Views\Store\Details.cshtml**檔案。
    ...
    *加入詳細資料檢視*
3. 修改**Details.cshtml**若要顯示的專輯資訊的檔案存取**專輯**傳遞至檢視範本的物件。 若要這樣做，請以下列取代內容：(C#)
...
### <a name="exercise-7-a-lap-around-aspnet-mvc-4-new-template"></a>練習 7:認識 ASP.NET MVC 4 的新範本
...
6. 現在您已經能夠探索解決方案，並查看一些推出由 ASP.NET MVC 4 專案範本的新功能。
   ...
   2. **包含的 JavaScript 程式庫**
      ...
      2. **jQuery UI**:此程式庫會提供低階互動和動畫、 進階的效果和主題化 widget，jQuery JavaScript 程式庫為基礎的抽象概念。

         > [!NOTE]
         > 您可以了解 jQuery 和 jQuery UI 中[ [ http://docs.jquery.com/ ](http://docs.jquery.com/) ](http://docs.jquery.com/)。
      3. **KnockoutJS**:ASP.NET MVC 4 預設範本現在包含**KnockoutJS**，可讓您建立豐富且回應迅速的 web 應用程式使用 JavaScript 和 HTML 的 JavaScript MVVM 架構。 例如在 ASP.NET MVC 3 中，jQuery 和 jQuery UI 程式庫也包含 ASP.NET MVC 4 中。
...
## <a name="appendix-b-publishing-an-aspnet-mvc-4-application-using-web-deploy"></a>附錄 b:發行 ASP.NET MVC 4 應用程式使用 Web Deploy
