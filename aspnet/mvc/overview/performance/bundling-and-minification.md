...
傳送的位元組必須大幅降低透過搭售，瀏覽器會使用其套用於要求的 HTTP 標頭相當詳細資訊。 接收的位元組減少不是最大因為最大的檔案 (*指令碼\\jquery-ui-1.8.11.min.js*並*指令碼\\jquery 1.7.1.min.js*) 已縮減. 注意:使用的範例程式計時[Fiddler](http://www.fiddler2.com/fiddler2/)來模擬慢速網路工具。 (從 Fiddler**規則**功能表上，選取**效能**然後**模擬數據機速度**。)
