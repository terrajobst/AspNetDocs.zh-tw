...
[建立 LINQ to SQL 類別的 ![](displaying-a-table-of-database-data-cs/_static/image1.jpg)](displaying-a-table-of-database-data-cs/_static/image1.png)

**圖 01**：建立 LINQ to SQL 類別（[按一下以觀看完整大小的影像](displaying-a-table-of-database-data-cs/_static/image2.png)）
...
**圖 02**：實體資料模型設計工具（[按一下以查看完整大小的影像](displaying-a-table-of-database-data-cs/_static/image4.png)）
...
[![[新增控制器] 功能表](displaying-a-table-of-database-data-cs/_static/image3.jpg)](displaying-a-table-of-database-data-cs/_static/image5.png)

**圖 03**： [新增控制器] 功能表（[按一下以查看完整大小的影像](displaying-a-table-of-database-data-cs/_static/image6.png)）
...
**圖 04**： [新增控制器] 對話方塊（[按一下以查看完整大小的影像](displaying-a-table-of-database-data-cs/_static/image8.png)）

我們需要修改 Movie controller 所公開的 Index （）動作，讓它傳回一組資料庫記錄。 修改控制器，使其看起來像 [清單 1] 中的控制器。

**清單1– Controllers\MovieController.cs**

[!code-csharp[Main](displaying-a-table-of-database-data-cs/samples/sample1.cs)]

在 [清單 1] 中，MoviesDBEntities 類別是用來代表 MoviesDB 資料庫。 若要使用這個類別，您必須匯入 MvcApplication1. 模型命名空間，如下所示：

使用 MvcApplication1;

運算式*實體。MovieSet. ToList （）* 會傳回來自電影資料庫資料表的所有電影集合。
...
[![新增視圖](displaying-a-table-of-database-data-cs/_static/image5.jpg)](displaying-a-table-of-database-data-cs/_static/image9.png)

**圖 05**：加入視圖（[按一下以觀看完整大小的影像](displaying-a-table-of-database-data-cs/_static/image10.png)）

在 [**加入視圖**] 對話方塊中，核取標示為 [**建立強型別的視圖**] 的核取方塊。 選取 Movie 類別做為**view 資料類別**。 選取 [*清單*] 做為**視圖內容**（請參閱 [圖 6]）。 選取這些選項會產生強型別視圖，以顯示電影清單。

[![[加入視圖] 對話方塊](displaying-a-table-of-database-data-cs/_static/image6.jpg)](displaying-a-table-of-database-data-cs/_static/image11.png)

**圖 06**： [加入視圖] 對話方塊（[按一下以查看完整大小的影像](displaying-a-table-of-database-data-cs/_static/image12.png)）
...
[![電影的資料表](displaying-a-table-of-database-data-cs/_static/image7.jpg)](displaying-a-table-of-database-data-cs/_static/image13.png)

**圖 07**：電影的資料表（[按一下以觀看完整大小的影像](displaying-a-table-of-database-data-cs/_static/image14.png)）
...
[清單 4] 中的視圖包含會逐一查看所有電影的 foreach 迴圈。 針對每個電影，MovieTemplate 部分是用來格式化電影。 MovieTemplate 是藉由呼叫 RenderPartial （） helper 方法來呈現。

修改過的索引表會呈現資料庫記錄的相同 HTML 資料表。 不過，此視圖已經大幅簡化。

RenderPartial （）方法與大多數其他 helper 方法不同，因為它不會傳回字串。 因此，您必須使用 &lt;% Html. RenderPartial （）來呼叫 RenderPartial （）方法;%&gt;，而不是 &lt;% = Html. RenderPartial （）;%&gt;。
