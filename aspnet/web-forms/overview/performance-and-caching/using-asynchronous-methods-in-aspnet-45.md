...
> 本教學課程將告訴您使用[Visual Studio Express 2012 For web](https://www.microsoft.com/visualstudio/11)建立異步 ASP.NET Web form 應用程式的基本概念，這是 Microsoft Visual Studio 的免費版本。 您也可以使用[Visual Studio 2012](https://www.microsoft.com/visualstudio/11)。 本教學課程包含下列各節。
> 
> - [執行緒集區處理要求的方式](#HowRequestsProcessedByTP)
> - [選擇同步或非同步方法](#ChoosingSyncVasync)
> - [範例應用程式](#SampleApp)
> - [[Gizmos 同步] 頁面](#GizmosSynch)
...
如果 web 應用程式在啟動時看到大量的並行要求，或有暴增的負載（並行成長突然增加），使 web 服務呼叫非同步會增加應用程式的回應能力。 非同步要求所需的處理時間與同步要求相同。 例如，如果要求提出需要兩秒完成的 web 服務呼叫，則要求需要兩秒鐘的時間，不論是以同步或非同步方式執行。 不過，在非同步呼叫期間，執行緒在等候第一個要求完成時，不會被封鎖而無法回應其他要求。 因此，當有多個並行要求會叫用長時間執行的作業時，非同步要求會阻止要求佇列和執行緒集區成長。
...
- 作業很簡單或執行時間短暫。
- 簡潔比效率重要。
- 作業主要都是 CPU 作業，而非需要耗用大量磁碟或網路的作業。 在 CPU 系結作業上使用非同步方法不會提供任何好處，而且會產生更多額外負荷。
...
- 當切換執行緒的好處高於內容切換的成本時。 一般來說，如果同步方法在不執行任何工作的情況下封鎖 ASP.NET 要求執行緒，您應該將方法設為非同步。 藉由讓呼叫非同步，ASP.NET 要求執行緒在等待 web 服務要求完成時，不會執行任何工作。
- 測試會顯示封鎖作業是網站效能的瓶頸，而 IIS 可以針對這些封鎖呼叫使用非同步方法來服務更多要求。

  可下載的範例會顯示如何有效地使用非同步方法。 提供的範例是設計來提供 ASP.NET 4.5 中非同步程式設計的簡單示範。 此範例不是為了在 ASP.NET 中進行非同步程式設計的參考架構。 範例程式會呼叫[ASP.NET Web API](../../../web-api/index.md)方法，然後再呼叫工作[。延遲](https://msdn.microsoft.com/library/hh139096(VS.110).aspx)以模擬長時間執行的 Web 服務呼叫。 大部分的生產應用程式都不會顯示使用非同步方法的明顯好處。   
  
少數應用程式要求所有方法都是非同步。 通常，將幾個同步方法轉換為非同步方法，可為所需的工作量增加最佳效率。
...
- *WebAppAsync*：使用 Web API **WebAPIpwg**服務的 ASP.NET Web Forms 專案。 本教學課程的大部分程式碼都來自于這個專案。
- *WebAPIpgw*：用來執行 `Products, Gizmos and Widgets` 控制器的 ASP.NET MVC 4 Web API 專案。 它會提供*WebAppAsync*專案和*Mvc4Async*專案的資料。
- *Mvc4Async*： ASP.NET MVC 4 專案，其中包含另一個教學課程中使用的程式碼。 它會對**WebAPIpwg**服務發出 Web API 呼叫。

## <a id="GizmosSynch"></a>[Gizmos 同步] 頁面

 下列程式碼顯示用來顯示 gizmos 清單的 `Page_Load` 同步方法。 （在本文中，gizmo 是虛構的機械裝置）。 
...
## <a id="CreatingAsynchGizmos"></a>建立異步 Gizmos 頁面
...
ASP.NET 非同步頁面必須包含[頁面](https://msdn.microsoft.com/library/ydy4x04a.aspx)指示詞，且 `Async` 屬性設定為 "true"。 下列程式碼顯示 GizmosAsync 屬性設定為 "true" 的 `Async` [Page](https://msdn.microsoft.com/library/ydy4x04a.aspx)指示詞，適用于 *.aspx*頁面。

[!code-aspx[Main](using-asynchronous-methods-in-aspnet-45/samples/sample3.aspx?highlight=1)]

下列程式碼顯示 `Gizmos` 同步 `Page_Load` 方法和 `GizmosAsync` 非同步網頁。 如果您的瀏覽器支援[HTML 5 &lt;mark&gt; 元素](http://www.w3.org/wiki/HTML/Elements/mark)，您會在黃色醒目提示的 `GizmosAsync` 中看到變更。
...
- 新的 `GetGizmosSvcAsync` 方法會以[async](https://msdn.microsoft.com/library/hh156513(VS.110).aspx)關鍵字標示，這會告訴編譯器為主體的部分產生回呼，並自動建立傳回的 `Task`。
- &quot;非同步&quot; 附加至非同步方法名稱。 不需要附加 "Async"，而是撰寫非同步方法時的慣例。
- 新 `GetGizmosSvcAsync` 方法的傳回型別為 `Task`。 `Task` 的傳回類型代表進行中的工作，並提供方法的呼叫端，以及等候非同步作業完成的控制碼。
- [Await](https://msdn.microsoft.com/library/hh156528(VS.110).aspx)關鍵字已套用至 web 服務呼叫。
- 已呼叫非同步 web 服務 API （`GetGizmosAsync`）。

在 `GetGizmosSvcAsync` 方法主體內，另一個非同步方法，`GetGizmosAsync` 呼叫。 `GetGizmosAsync` 立即傳回當資料可供使用時，最後會完成的 `Task<List<Gizmo>>`。 因為您不想要在擁有 gizmo 資料之前執行任何其他動作，所以程式碼會等候工作（使用**await**關鍵字）。 您只能在以**async**關鍵字標注的方法中使用**await**關鍵字。
...
- 會使用非同步[HttpClient](https://msdn.microsoft.com/library/system.net.http.httpclient(VS.110).aspx)類別，而不是同步的[WebClient](https://msdn.microsoft.com/library/system.net.webclient.aspx)類別。
...
## <a name="registerasynctask-notes"></a>RegisterAsyncTask 附注

與 `RegisterAsyncTask`[連結的方法將會在建立](https://msdn.microsoft.com/library/ms178472.aspx)後立即執行。 您也可以直接使用 async void page 事件，如下列程式碼所示：

[!code-csharp[Main](using-asynchronous-methods-in-aspnet-45/samples/sample8.cs)]

Async void 事件的缺點是，開發人員不再具有事件執行時間的完整控制權。 例如，如果 .aspx 和都是。Master 定義 `Page_Load` 事件，其中一個或兩個都是非同步，因此無法保證執行的順序。 適用于非事件處理常式的相同 indeterminiate 順序（如 `async void Button_Click`）。 對於大部分的開發人員而言，這應該是可接受的，但需要完全控制執行順序的使用者，應該只使用 Api （例如 `RegisterAsyncTask`）來取用會傳回 Task 物件的方法。

## <a id="Parallel"></a>平行執行多個作業

當動作必須執行數個獨立的作業時，非同步方法與同步方法具有顯著優勢。 在提供的範例中，同步頁面*PWG*（適用于產品、Widget 和 Gizmos）會顯示三個 web 服務呼叫的結果，以取得產品、widget 和 Gizmos 的清單。 提供這些服務的[ASP.NET Web API](../../../web-api/index.md)專案會使用 [[延遲](https://msdn.microsoft.com/library/hh139096(VS.110).aspx)] 來模擬延遲或緩慢的網路呼叫。 當延遲時間設定為500毫秒時，非同步*PWGasync .aspx*頁面會花費超過500毫秒來完成，而同步 `PWG` 版本則需要超過1500毫秒。 [同步*PWG* ] 頁面會顯示在下列程式碼中。

[!code-csharp[Main](using-asynchronous-methods-in-aspnet-45/samples/sample9.cs)]

非同步 `PWGasync` 程式碼後置如下所示。

[!code-csharp[Main](using-asynchronous-methods-in-aspnet-45/samples/sample10.cs?highlight=5,11,21)]

下圖顯示從非同步*PWGasync*頁面傳回的視圖。

![](using-asynchronous-methods-in-aspnet-45/_static/image3.png)

## <a id="CancelToken"></a>使用解除標記

傳回 `Task`的非同步方法是可取消的，而當[頁面](https://msdn.microsoft.com/library/ydy4x04a.aspx)指示詞的 `AsyncTimeout` 屬性提供時，就會接受[CancellationToken](https://msdn.microsoft.com/library/system.threading.cancellationtoken(VS.110).aspx)參數。 下列程式碼顯示 GizmosCancelAsync 為（秒）的*default.aspx*頁面。
...
在提供的範例應用程式中，選取 [ *GizmosCancelAsync* ] 連結會呼叫*GizmosCancelAsync*頁面，並示範非同步呼叫的取消（藉由計時）。 因為延遲時間是在隨機範圍內，所以您可能需要重新整理頁面幾次，才能取得逾時錯誤訊息。

## <a id="ServerConfig"></a>高平行存取/高延遲 Web 服務呼叫的伺服器設定

若要實現非同步 web 應用程式的優點，您可能需要對預設伺服器設定進行一些變更。 設定和壓力測試非同步 web 應用程式時，請記住下列事項。

- Windows 7、Windows Vista、Window 8 和所有 Windows 用戶端作業系統最多可以有10個並行要求。 您需要 Windows Server 作業系統，才能在高負載下查看非同步方法的優點。
