...
第一個步驟是建立網站的主版頁面。 現在，我們的網站只包含具類型的資料集（`Northwind.xsd`、`App_Code` 資料夾中）、BLL 類別（`ProductsBLL.vb`、`CategoriesBLL.vb`等等、全部在 `App_Code` 資料夾中）、設定檔（`NORTHWND.MDF`），以及 CSS 樣式表單檔案（`App_Data`）。 我從前兩個教學課程中，清除了使用 DAL 和 BLL 示範的頁面和檔案，因為我們將在未來的教學課程中更詳細地 reexamining 這些範例。
