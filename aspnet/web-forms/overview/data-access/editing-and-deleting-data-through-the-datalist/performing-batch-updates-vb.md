...
[![E您可以修改 ach 中完整的可編輯 DataList 項目](performing-batch-updates-vb/_static/image2.png)](performing-batch-updates-vb/_static/image1.png)
...
## <a name="step-1-create-the-editable-user-interface-in-the-datalist-s-itemtemplate"></a>步驟 1：DataList 的 ItemTemplate 中建立的可編輯的使用者介面
...
[![Create 新 ObjectDataSource 名為 SuppliersDataSource](performing-batch-updates-vb/_static/image5.png)](performing-batch-updates-vb/_static/image4.png)
...
[![Retrieve 使用 GetSuppliers() 方法的供應商資訊](performing-batch-updates-vb/_static/image8.png)](performing-batch-updates-vb/_static/image7.png)

**圖 3**:擷取資訊所使用的供應商`GetSuppliers()`方法 ([按一下以檢視完整大小的影像](performing-batch-updates-vb/_static/image9.png))


[![Set 為 (None) 下拉式清單中 [更新] 索引標籤](performing-batch-updates-vb/_static/image11.png)](performing-batch-updates-vb/_static/image10.png)
...
[![E除此之外，每個供應商 DataList 中是可編輯](performing-batch-updates-vb/_static/image14.png)](performing-batch-updates-vb/_static/image13.png)
...
[![Two 更新所有按鈕已都加入至頁面](performing-batch-updates-vb/_static/image17.png)](performing-batch-updates-vb/_static/image16.png)
...
用於此教學課程的呼叫的批次更新演算法`UpdateSupplierAddress`方法*每個*DataList，不論是否已變更其位址資訊中的供應商。 而這類盲目更新通常不是效能問題，它們可能會導致過多記錄您稽核變更為資料庫資料表。 例如，如果您使用觸發程序來記錄所有`UPDATE`s`Suppliers`資料表至稽核資料表，每次使用者按一下 [全部更新] 按鈕，為每個供應商在系統中，不論使用者是否進行任何中建立新的稽核記錄變更。
