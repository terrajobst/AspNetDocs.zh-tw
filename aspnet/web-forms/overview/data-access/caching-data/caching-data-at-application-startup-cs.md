...
我們已實作為日期的 Northwind 資料庫資料表不包含任何傳統查閱資料表。 這四個 Datatable 在 DAL 中實作為所有模型資料表，其值為非靜態。 本教學課程只是假設 `Suppliers` 資料表的資料是靜態的，而不是花時間將新的 DataTable 加入 DAL，再加上新的類別和方法到 BLL。 因此，我們可以在應用程式啟動時快取此資料。
...
上述程式碼會使用靜態成員變數 `suppliers`，以保存來自 `SuppliersBLL` 類別的 `GetSuppliers()` 方法（從 `LoadStaticCache()` 方法呼叫）的結果。 `LoadStaticCache()` 方法是要在應用程式啟動期間呼叫。 一旦在應用程式啟動時載入此資料，任何需要使用供應商資料的頁面都可以呼叫 `StaticCache` 類別的 `GetSuppliers()` 方法。 因此，在應用程式啟動時，對資料庫的呼叫只會出現一次。
...
若要將專案新增至資料快取，但沒有以時間為基礎的到期日，請使用 `System.Web.Caching.Cache.NoAbsoluteExpiration` 和 `System.Web.Caching.Cache.NoSlidingExpiration` 值做為輸入參數。 已選取此資料快取 `Insert` 方法的特定多載，因此我們可以指定快取專案的*優先順序*。 優先順序是用來決定可用記憶體不足時，要從快取中清除哪些專案。 在此，我們使用優先順序 `NotRemovable`，以確保不會清除此快取專案。

> [!NOTE]
> 本教學課程的下載會使用靜態成員變數方法來實行 `StaticCache` 類別。 應用程式狀態和資料快取技術的程式碼可在類別檔案的批註中取得。
...
以滑鼠右鍵按一下 Visual Studio 的方案總管中的網站專案名稱，然後選擇 [加入新專案]，將 `Global.asax` 檔案加入至 web 應用程式的根目錄。 從 [加入新專案] 對話方塊中，選取 [全域應用程式類別] 專案類型，然後按一下 [加入] 按鈕。
...
**圖 3**：將 `Global.asax` 檔案新增至 Web 應用程式的根目錄（[按一下以查看完整大小的影像](caching-data-at-application-startup-cs/_static/image5.png)）
...
在這些教學課程中，我們只需要將程式碼新增至 `Application_Start` 方法，因此您可以自由移除其他專案。 在 `Application_Start`中，只要呼叫 `StaticCache` 類別的 `LoadStaticCache()` 方法，就會載入並快取供應商資訊：

[!code-aspx[Main](caching-data-at-application-startup-cs/samples/sample6.aspx)]

這樣就全部完成了！ 在應用程式啟動時，`LoadStaticCache()` 方法會從 BLL 抓取供應商資訊，並將其儲存在靜態成員變數中（或您在 `StaticCache` 類別中使用的任何快取存放區）。 若要確認此行為，請在 `Application_Start` 方法中設定中斷點，並執行您的應用程式。 請注意，啟動應用程式時，會叫用中斷點。 不過，後續要求並不會造成 `Application_Start` 方法執行。
...
**圖 4**：使用中斷點來確認正在執行 `Application_Start` 事件處理常式（[按一下以查看完整大小的影像](caching-data-at-application-startup-cs/_static/image8.png)）
...
此時，`StaticCache` 類別具有在應用程式啟動時快取的供應商資料版本，可以透過其 `GetSuppliers()` 方法來存取。 若要從展示層使用此資料，我們可以使用 ObjectDataSource，或從 ASP.NET 網頁的程式碼後置類別中，以程式設計方式叫用 `StaticCache` 類別的 `GetSuppliers()` 方法。 讓我們看看如何使用 ObjectDataSource 和 GridView 控制項來顯示快取的供應商資訊。
...
**圖 6**：使用 `GetSuppliers()` 方法來抓取快取的供應商資料（[按一下以查看完整大小的影像](caching-data-at-application-startup-cs/_static/image14.png)）
...
[圖 7] 顯示透過瀏覽器觀看的頁面。 輸出與我們從 BLL 的 `SuppliersBLL` 類別提取資料的方式相同，但使用 `StaticCache` 類別會傳回供應商資料，如在應用程式啟動時快取。 您可以在 `StaticCache` 類別的 `GetSuppliers()` 方法中設定中斷點，以驗證此行為。
