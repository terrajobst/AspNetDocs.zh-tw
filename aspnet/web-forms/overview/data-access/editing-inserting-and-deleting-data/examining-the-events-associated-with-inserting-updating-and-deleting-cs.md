...
若要設定 ObjectDataSource，選取設定資料來源連結，從它的智慧標籤，啟動精靈。 第一個畫面可讓您變更基礎物件的 ObjectDataSource 會繫結至;將它設定為`ProductsBLL`。 下一個畫面中列出的基礎物件的從 ObjectDataSource 的方法的對應。 我們會明確指出，雖然`Insert()`和`Delete()`方法不應對應至任何方法，如果您移至 插入和刪除索引標籤會看到對應是否有。 這是因為`ProductsBLL`的`AddProduct`並`DeleteProduct`方法會使用`DataObjectMethodAttribute`屬性，表示它們是預設方法，如`Insert()`和`Delete()`分別。 因此，ObjectDataSource 精靈會選取這些每次您執行精靈，除非有明確指定其他值。
