...
若要將新的預存程式加入至專案，請以滑鼠右鍵按一下 `ManagedDatabaseConstructs` 專案名稱，然後加入宣告新的預存程式。 將檔案命名為 `GetProductsWithPriceLessThan.cs`。 如我們在步驟3中所見，這會建立C#新的類別檔案，其中包含名為的方法 `GetProductsWithPriceLessThan` 放在 `partial` 類別 `StoredProcedures`內。
