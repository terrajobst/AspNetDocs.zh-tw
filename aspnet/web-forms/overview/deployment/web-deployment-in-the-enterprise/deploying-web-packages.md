...
不論您是否執行部署網頁套件 *。 deploy.cmd*檔案，或藉由直接使用 MSDeploy.exe，您需要指定驗證類型。 Web Deploy 可接受兩個可能值：**NTLM**或是**基本**。 如果您指定基本驗證時，您也需要提供使用者名稱和密碼。 有各種因素，您必須要知道當您選取驗證類型：
