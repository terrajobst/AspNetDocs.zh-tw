...
## <a name="adding-order-tracking"></a><span data-ttu-id="86b8b-127">新增訂單追蹤</span><span class="sxs-lookup"><span data-stu-id="86b8b-127">Adding Order Tracking</span></span>
...
<span data-ttu-id="86b8b-179">請務必注意，ASP.NET 設定會遵循階層，其中每個*web.config*檔案會將設定值套用至其所在的資料夾，以及其底下的所有子目錄。</span><span class="sxs-lookup"><span data-stu-id="86b8b-179">It's important to note that ASP.NET configuration follows a hierarchy, where each *Web.config* file applies configuration settings to the folder that it is in and to all of the child directories below it.</span></span>
...
10. <span data-ttu-id="86b8b-201">按一下 **[是]** ，將憑證安裝到您的 localhost。</span><span class="sxs-lookup"><span data-stu-id="86b8b-201">Click **Yes** to install the certificate to your localhost.</span></span>   
...
9. <span data-ttu-id="86b8b-236">將授權的重新**導向 URI**設定為下列內容：</span><span class="sxs-lookup"><span data-stu-id="86b8b-236">Set the **Authorized Redirect URI** to the following:</span></span> 
   ...
   <span data-ttu-id="86b8b-237">此值是 ASP.NET OAuth 使用者與 google OAuth 伺服器通訊的 URI。</span><span class="sxs-lookup"><span data-stu-id="86b8b-237">This value is the URI that ASP.NET OAuth users to communicate with the google OAuth server.</span></span> <span data-ttu-id="86b8b-238">請記住您先前使用的 SSL URL （除非您已建立其他 SSL 專案，否則 `https://localhost:44300/`）。</span><span class="sxs-lookup"><span data-stu-id="86b8b-238">Remember the SSL URL you used above (    `https://localhost:44300/` unless you've created other SSL projects).</span></span>
...
<span data-ttu-id="86b8b-278">除了修改*Login.aspx.cs*程式碼後置檔案，以在使用者登入時遷移購物車時，您也必須修改*Register.aspx.cs 程式碼後*置檔案，以在使用者建立新帳戶並登入時遷移購物車。</span><span class="sxs-lookup"><span data-stu-id="86b8b-278">In addition to modifying the *Login.aspx.cs* code-behind file to migrate the shopping cart when the user logs in, you must also modify the *Register.aspx.cs code-behind file* to migrate the shopping cart when the user creates a new account and logs in.</span></span>
...
### <a name="add-paypal-class-and-api-credentials"></a><span data-ttu-id="86b8b-329">新增 PayPal 類別和 API 認證</span><span class="sxs-lookup"><span data-stu-id="86b8b-329">Add PayPal Class and API Credentials</span></span>
...
- <span data-ttu-id="86b8b-347">`SetExpressCheckout` 函式</span><span class="sxs-lookup"><span data-stu-id="86b8b-347">`SetExpressCheckout` function</span></span>
...
### <a name="update-the-localhost-port-number-in-the-paypal-class"></a><span data-ttu-id="86b8b-362">更新 PayPal 類別中的 LocalHost 埠號碼</span><span class="sxs-lookup"><span data-stu-id="86b8b-362">Update the LocalHost Port Number in the PayPal Class</span></span>
...
### <a name="review-order-details"></a><span data-ttu-id="86b8b-394">審核順序詳細資料</span><span class="sxs-lookup"><span data-stu-id="86b8b-394">Review Order Details</span></span>
...
> 
> <span data-ttu-id="86b8b-408">藉由選取 [**編輯欄位**]，將會顯示 [**欄位**] 對話方塊。</span><span class="sxs-lookup"><span data-stu-id="86b8b-408">By selecting **Edit Fields**, the **Fields** dialog box will appear.</span></span> <span data-ttu-id="86b8b-409">在此對話方塊中，您可以輕鬆地控制**DetailsView**控制項的視覺屬性，例如**ItemStyle**。</span><span class="sxs-lookup"><span data-stu-id="86b8b-409">In this dialog box you can easily control the visual properties, such as **ItemStyle**, of the **DetailsView** control.</span></span>
> 
> ![使用 PayPal-[欄位] 對話方塊簽出和付款](checkout-and-payment-with-paypal/_static/image19.png)

### <a name="complete-purchase"></a><span data-ttu-id="86b8b-411">完成購買</span><span class="sxs-lookup"><span data-stu-id="86b8b-411">Complete Purchase</span></span>

<span data-ttu-id="86b8b-412">*CheckoutComplete .aspx*頁面會向 PayPal 購買。</span><span class="sxs-lookup"><span data-stu-id="86b8b-412">*CheckoutComplete.aspx* page makes the purchase from PayPal.</span></span> <span data-ttu-id="86b8b-413">如上所述，使用者必須按一下 [**完整訂單**] 按鈕，應用程式才會流覽至*CheckoutComplete .aspx*頁面。</span><span class="sxs-lookup"><span data-stu-id="86b8b-413">As mentioned above, the user must click on the **Complete Order** button before the application will navigate to the *CheckoutComplete.aspx* page.</span></span>

1. <span data-ttu-id="86b8b-414">在 [*簽出*] 資料夾中，開啟名為*CheckoutComplete*的頁面。</span><span class="sxs-lookup"><span data-stu-id="86b8b-414">In the *Checkout* folder, open the page named *CheckoutComplete.aspx*.</span></span>
2. <span data-ttu-id="86b8b-415">以下列內容取代現有的標記：</span><span class="sxs-lookup"><span data-stu-id="86b8b-415">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample22.aspx)]
3. <span data-ttu-id="86b8b-416">開啟名為*CheckoutComplete.aspx.cs*的程式碼後置頁面，並將現有的程式碼取代為下列內容：</span><span class="sxs-lookup"><span data-stu-id="86b8b-416">Open the code-behind page named *CheckoutComplete.aspx.cs* and replace the existing code with the following:</span></span>   

    [!code-csharp[Main](checkout-and-payment-with-paypal/samples/sample23.cs)]

<span data-ttu-id="86b8b-417">載入*CheckoutComplete .aspx*頁面時，會呼叫 `DoCheckoutPayment` 方法。</span><span class="sxs-lookup"><span data-stu-id="86b8b-417">When the *CheckoutComplete.aspx* page is loaded, the `DoCheckoutPayment` method is called.</span></span> <span data-ttu-id="86b8b-418">如先前所述，`DoCheckoutPayment` 方法會從 PayPal 測試環境完成購買。</span><span class="sxs-lookup"><span data-stu-id="86b8b-418">As mentioned earlier, the `DoCheckoutPayment` method completes the purchase from the PayPal testing environment.</span></span> <span data-ttu-id="86b8b-419">一旦 PayPal 完成訂單的購買後，[ *CheckoutComplete* ] 頁面就會顯示購買者 `ID` 的付款交易。</span><span class="sxs-lookup"><span data-stu-id="86b8b-419">Once PayPal has completed the purchase of the order, the *CheckoutComplete.aspx* page displays a payment transaction `ID` to the purchaser.</span></span>

### <a name="handle-cancel-purchase"></a><span data-ttu-id="86b8b-420">處理取消購買</span><span class="sxs-lookup"><span data-stu-id="86b8b-420">Handle Cancel Purchase</span></span>

<span data-ttu-id="86b8b-421">如果使用者決定取消購買，他們會被導向至 [ *CheckoutCancel* ] 頁面，他們會看到他們的訂單已取消。</span><span class="sxs-lookup"><span data-stu-id="86b8b-421">If the user decides to cancel the purchase, they will be directed to the *CheckoutCancel.aspx* page where they will see that their order has been cancelled.</span></span>

1. <span data-ttu-id="86b8b-422">在 [*簽出*] 資料夾中，開啟名為*CheckoutCancel*的頁面。</span><span class="sxs-lookup"><span data-stu-id="86b8b-422">Open the page named *CheckoutCancel.aspx* in the *Checkout* folder.</span></span>
2. <span data-ttu-id="86b8b-423">以下列內容取代現有的標記：</span><span class="sxs-lookup"><span data-stu-id="86b8b-423">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample24.aspx)]

### <a name="handle-purchase-errors"></a><span data-ttu-id="86b8b-424">處理購買錯誤</span><span class="sxs-lookup"><span data-stu-id="86b8b-424">Handle Purchase Errors</span></span>

<span data-ttu-id="86b8b-425">在購買程式期間的錯誤將由*CheckoutError*處理。</span><span class="sxs-lookup"><span data-stu-id="86b8b-425">Errors during the purchase process will be handled by the *CheckoutError.aspx* page.</span></span> <span data-ttu-id="86b8b-426">*CheckoutStart*的程式碼後置、 *CheckoutReview*頁面和*CheckoutComplete*頁面會在發生錯誤時，每個都會重新導向至*CheckoutError .aspx*頁面。</span><span class="sxs-lookup"><span data-stu-id="86b8b-426">The code-behind of the *CheckoutStart.aspx* page, the *CheckoutReview.aspx* page, and the *CheckoutComplete.aspx* page will each redirect to the *CheckoutError.aspx* page if an error occurs.</span></span>

1. <span data-ttu-id="86b8b-427">在 [*簽出*] 資料夾中，開啟名為*CheckoutError*的頁面。</span><span class="sxs-lookup"><span data-stu-id="86b8b-427">Open the page named *CheckoutError.aspx* in the *Checkout* folder.</span></span>
2. <span data-ttu-id="86b8b-428">以下列內容取代現有的標記：</span><span class="sxs-lookup"><span data-stu-id="86b8b-428">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample25.aspx)]

<span data-ttu-id="86b8b-429">當簽出程式期間發生錯誤時，會顯示 [ *CheckoutError* ] 頁面，其中包含錯誤詳細資料。</span><span class="sxs-lookup"><span data-stu-id="86b8b-429">The *CheckoutError.aspx* page is displayed with the error details when an error occurs during the checkout process.</span></span>

## <a name="running-the-application"></a><span data-ttu-id="86b8b-430">執行應用程式</span><span class="sxs-lookup"><span data-stu-id="86b8b-430">Running the Application</span></span>

<span data-ttu-id="86b8b-431">執行應用程式，以瞭解如何購買產品。</span><span class="sxs-lookup"><span data-stu-id="86b8b-431">Run the application to see how to purchase products.</span></span> <span data-ttu-id="86b8b-432">請注意，您將會在 PayPal 測試環境中執行。</span><span class="sxs-lookup"><span data-stu-id="86b8b-432">Note that you will be running in the PayPal testing environment.</span></span> <span data-ttu-id="86b8b-433">不會交換實際的金錢。</span><span class="sxs-lookup"><span data-stu-id="86b8b-433">No actual money is being exchanged.</span></span>

1. <span data-ttu-id="86b8b-434">請確定所有檔案都儲存在 Visual Studio 中。</span><span class="sxs-lookup"><span data-stu-id="86b8b-434">Make sure all your files are saved in Visual Studio.</span></span>
2. <span data-ttu-id="86b8b-435">開啟網頁瀏覽器，並流覽至[https://developer.paypal.com](https://developer.paypal.com/)。</span><span class="sxs-lookup"><span data-stu-id="86b8b-435">Open a Web browser and navigate to [https://developer.paypal.com](https://developer.paypal.com/).</span></span>
3. <span data-ttu-id="86b8b-436">使用您稍早在本教學課程中建立的 PayPal 開發人員帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="86b8b-436">Login with your PayPal developer account that you created earlier in this tutorial.</span></span>  
   <span data-ttu-id="86b8b-437">對於 PayPal 的開發人員沙箱，您必須在[https://developer.paypal.com](https://developer.paypal.com/)登入，以測試快速簽出。</span><span class="sxs-lookup"><span data-stu-id="86b8b-437">For PayPal's developer sandbox, you need to be logged in at [https://developer.paypal.com](https://developer.paypal.com/) to test express checkout.</span></span> <span data-ttu-id="86b8b-438">這僅適用于 PayPal 的沙箱測試，而不適用於 PayPal 的即時環境。</span><span class="sxs-lookup"><span data-stu-id="86b8b-438">This only applies to PayPal's sandbox testing, not to PayPal's live environment.</span></span>
4. <span data-ttu-id="86b8b-439">在 Visual Studio 中，按**F5**執行 Wingtip 玩具範例應用程式。</span><span class="sxs-lookup"><span data-stu-id="86b8b-439">In Visual Studio, press **F5** to run the Wingtip Toys sample application.</span></span>  
   <span data-ttu-id="86b8b-440">重建資料庫之後，瀏覽器將會開啟並顯示*default.aspx*頁面。</span><span class="sxs-lookup"><span data-stu-id="86b8b-440">After the database rebuilds, the browser will open and show the *Default.aspx* page.</span></span>
5. <span data-ttu-id="86b8b-441">藉由選取產品類別（例如「汽車」），然後按一下每個產品旁的 [**新增至購物車**]，將三個不同的產品新增至購物車。</span><span class="sxs-lookup"><span data-stu-id="86b8b-441">Add three different products to the shopping cart by selecting the product category, such as "Cars" and then clicking **Add to Cart** next to each product.</span></span>  
   <span data-ttu-id="86b8b-442">購物車會顯示您所選取的產品。</span><span class="sxs-lookup"><span data-stu-id="86b8b-442">The shopping cart will display the product you have selected.</span></span>
6. <span data-ttu-id="86b8b-443">按一下 [ **PayPal** ] 按鈕以簽出。</span><span class="sxs-lookup"><span data-stu-id="86b8b-443">Click the **PayPal** button to checkout.</span></span> 

    ![使用 PayPal 的結帳和付款-購物車](checkout-and-payment-with-paypal/_static/image20.png)

   <span data-ttu-id="86b8b-445">簽出將會要求您必須擁有 Wingtip 玩具範例應用程式的使用者帳戶。</span><span class="sxs-lookup"><span data-stu-id="86b8b-445">Checking out will require that you have a user account for the Wingtip Toys sample application.</span></span>
7. <span data-ttu-id="86b8b-446">按一下頁面右側的 [ **Google** ] 連結，以使用現有的 gmail.com 電子郵件帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="86b8b-446">Click the **Google** link on the right of the page to log in with an existing gmail.com email account.</span></span>  
   <span data-ttu-id="86b8b-447">如果您沒有 gmail.com 帳戶，可以在[www.gmail.com](https://www.gmail.com/)建立一個用於測試的用途。</span><span class="sxs-lookup"><span data-stu-id="86b8b-447">If you do not have a gmail.com account, you can create one for testing purposes at [www.gmail.com](https://www.gmail.com/).</span></span> <span data-ttu-id="86b8b-448">您也可以按一下 [註冊]，以使用標準的本機帳戶。</span><span class="sxs-lookup"><span data-stu-id="86b8b-448">You can also use a standard local account by clicking "Register".</span></span> 

    ![簽出與使用 PayPal 付款-登入](checkout-and-payment-with-paypal/_static/image21.png)
8. <span data-ttu-id="86b8b-450">使用您的 gmail 帳戶和密碼登入。</span><span class="sxs-lookup"><span data-stu-id="86b8b-450">Sign in with your gmail account and password.</span></span> 

    ![使用 PayPal 進行簽出和付款-Gmail 登入](checkout-and-payment-with-paypal/_static/image22.png)
9. <span data-ttu-id="86b8b-452">按一下 [**登入**] 按鈕，向您的 Wingtip 玩具範例應用程式使用者名稱註冊您的 gmail 帳戶。</span><span class="sxs-lookup"><span data-stu-id="86b8b-452">Click the **Log in** button to register your gmail account with your Wingtip Toys sample application user name.</span></span> 

    ![使用 PayPal 簽出和付款-註冊帳戶](checkout-and-payment-with-paypal/_static/image23.png)
10. <span data-ttu-id="86b8b-454">在 PayPal 測試網站上，新增您稍早在本教學課程中建立的**買方**電子郵件地址和密碼，然後按一下 [**登入**] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="86b8b-454">On the PayPal test site, add your **buyer** email address and password that you created earlier in this tutorial, then click the **Log In** button.</span></span> 

    ![使用 PayPal 簽出和付款-PayPal 登入](checkout-and-payment-with-paypal/_static/image24.png)
11. <span data-ttu-id="86b8b-456">同意 PayPal 原則，然後按一下 [**同意並繼續**] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="86b8b-456">Agree to the PayPal policy and click the **Agree and Continue** button.</span></span>  
    <span data-ttu-id="86b8b-457">請注意，只有當您第一次使用此 PayPal 帳戶時，才會顯示此頁面。</span><span class="sxs-lookup"><span data-stu-id="86b8b-457">Note that this page is only displayed the first time you use this PayPal account.</span></span> <span data-ttu-id="86b8b-458">同樣地，請注意這是測試帳戶，並不會交換真正的金錢。</span><span class="sxs-lookup"><span data-stu-id="86b8b-458">Again note that this is a test account, no real money is exchanged.</span></span> 

    ![使用 PayPal 簽出和付款-PayPal 原則](checkout-and-payment-with-paypal/_static/image25.png)
12. <span data-ttu-id="86b8b-460">請參閱 PayPal 測試環境審查頁面上的訂單資訊，然後按一下 [**繼續**]。</span><span class="sxs-lookup"><span data-stu-id="86b8b-460">Review the order information on the PayPal testing environment review page and click **Continue**.</span></span> 

    ![簽出與使用 PayPal 的款項-審查資訊](checkout-and-payment-with-paypal/_static/image26.png)
13. <span data-ttu-id="86b8b-462">在 [ *CheckoutReview* ] 頁面上，確認訂單金額，並查看產生的寄送位址。</span><span class="sxs-lookup"><span data-stu-id="86b8b-462">On the *CheckoutReview.aspx* page, verify the order amount and view the generated shipping address.</span></span> <span data-ttu-id="86b8b-463">然後，按一下 [**完成訂單**] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="86b8b-463">Then, click the **Complete Order** button.</span></span> 

    ![使用 PayPal-訂單審查簽出和付款](checkout-and-payment-with-paypal/_static/image27.png)
14. <span data-ttu-id="86b8b-465">隨即會顯示 [ **CheckoutComplete** ] 頁面，其中包含付款交易識別碼。</span><span class="sxs-lookup"><span data-stu-id="86b8b-465">The **CheckoutComplete.aspx** page is displayed with a payment transaction ID.</span></span> 

    ![使用 PayPal 簽出和付款-結帳完成](checkout-and-payment-with-paypal/_static/image28.png)

<a id="ReviewDBWebForms"></a>
## <a name="reviewing-the-database"></a><span data-ttu-id="86b8b-467">檢查資料庫</span><span class="sxs-lookup"><span data-stu-id="86b8b-467">Reviewing the Database</span></span>

<span data-ttu-id="86b8b-468">在執行應用程式後，藉由在 Wingtip 玩具範例應用程式資料庫中查看更新的資料，您可以看到應用程式已成功記錄產品的購買。</span><span class="sxs-lookup"><span data-stu-id="86b8b-468">By reviewing the updated data in the Wingtip Toys sample application database after running the application, you can see that the application successfully recorded the purchase of the products.</span></span>

<span data-ttu-id="86b8b-469">您可以使用 [**資料庫總管**] 視窗（Visual Studio 中的 [**伺服器總管**] 視窗），檢查*Wingtiptoys*中包含的資料，就像您先前在本教學課程系列中所做的一樣。</span><span class="sxs-lookup"><span data-stu-id="86b8b-469">You can inspect the data contained in the *Wingtiptoys.mdf* database file by using the **Database Explorer** window (**Server Explorer** window in Visual Studio) as you did earlier in this tutorial series.</span></span>

1. <span data-ttu-id="86b8b-470">如果瀏覽器視窗仍為開啟狀態，請將它關閉。</span><span class="sxs-lookup"><span data-stu-id="86b8b-470">Close the browser window if it is still open.</span></span>
2. <span data-ttu-id="86b8b-471">在 Visual Studio 中，選取**方案總管**頂端的 [**顯示所有**檔案] 圖示，以讓您展開**應用程式\_** 資料夾。</span><span class="sxs-lookup"><span data-stu-id="86b8b-471">In Visual Studio, select the **Show All Files** icon at the top of **Solution Explorer** to allow you to expand the **App\_Data** folder.</span></span>
3. <span data-ttu-id="86b8b-472">展開**應用程式\_[Data** ] 資料夾。</span><span class="sxs-lookup"><span data-stu-id="86b8b-472">Expand the **App\_Data** folder.</span></span>  
 <span data-ttu-id="86b8b-473">您可能需要選取資料夾的 [**顯示所有**檔案] 圖示。</span><span class="sxs-lookup"><span data-stu-id="86b8b-473">You may need to select the **Show All Files** icon for the folder.</span></span>
4. <span data-ttu-id="86b8b-474">以滑鼠右鍵按一下*Wingtiptoys .mdf*資料庫檔案，然後選取 [**開啟**]。</span><span class="sxs-lookup"><span data-stu-id="86b8b-474">Right-click the *Wingtiptoys.mdf* database file and select **Open**.</span></span>  
    <span data-ttu-id="86b8b-475">隨即顯示 [**伺服器總管**]。</span><span class="sxs-lookup"><span data-stu-id="86b8b-475">**Server Explorer** is displayed.</span></span>
5. <span data-ttu-id="86b8b-476">展開 [**資料表]** 資料夾。</span><span class="sxs-lookup"><span data-stu-id="86b8b-476">Expand the **Tables** folder.</span></span>
6. <span data-ttu-id="86b8b-477">以滑鼠右鍵按一下**Orders**資料表，然後選取 [**顯示資料表資料**]。</span><span class="sxs-lookup"><span data-stu-id="86b8b-477">Right-click the **Orders**table and select **Show Table Data**.</span></span>  
 <span data-ttu-id="86b8b-478">[**訂單**] 資料表隨即顯示。</span><span class="sxs-lookup"><span data-stu-id="86b8b-478">The **Orders** table is displayed.</span></span>
7. <span data-ttu-id="86b8b-479">檢查**PaymentTransactionID**資料行以確認交易成功。</span><span class="sxs-lookup"><span data-stu-id="86b8b-479">Review the **PaymentTransactionID** column to confirm successful transactions.</span></span> 

    ![簽出和付款使用 PayPal-審核資料庫](checkout-and-payment-with-paypal/_static/image29.png)
8. <span data-ttu-id="86b8b-481">關閉 [ **Orders** ] 資料表視窗。</span><span class="sxs-lookup"><span data-stu-id="86b8b-481">Close the **Orders** table window.</span></span>
9. <span data-ttu-id="86b8b-482">在 伺服器總管中，以滑鼠右鍵按一下**OrderDetails**資料表，然後選取 **顯示資料表資料**。</span><span class="sxs-lookup"><span data-stu-id="86b8b-482">In the Server Explorer, right-click the **OrderDetails** table and select **Show Table Data**.</span></span>
10. <span data-ttu-id="86b8b-483">檢查**OrderDetails**資料表中的 `OrderId` 和 `Username` 值。</span><span class="sxs-lookup"><span data-stu-id="86b8b-483">Review the `OrderId` and `Username` values in the **OrderDetails** table.</span></span> <span data-ttu-id="86b8b-484">請注意，這些值符合**Orders**資料表中包含的 `OrderId` 和 `Username` 值。</span><span class="sxs-lookup"><span data-stu-id="86b8b-484">Note that these values match the `OrderId` and `Username` values included in the **Orders** table.</span></span>
11. <span data-ttu-id="86b8b-485">關閉 [ **OrderDetails**資料表] 視窗。</span><span class="sxs-lookup"><span data-stu-id="86b8b-485">Close the **OrderDetails** table window.</span></span>
12. <span data-ttu-id="86b8b-486">以滑鼠右鍵按一下 Wingtip 玩具資料庫檔案（*Wingtiptoys .mdf*），然後選取 [**關閉連接**]。</span><span class="sxs-lookup"><span data-stu-id="86b8b-486">Right-click the Wingtip Toys database file (*Wingtiptoys.mdf*) and select **Close Connection**.</span></span>
13. <span data-ttu-id="86b8b-487">如果您看不到 **方案總管** 視窗，請按一下 **伺服器總管** 視窗底部的 **方案總管**，再次顯示**方案總管**。</span><span class="sxs-lookup"><span data-stu-id="86b8b-487">If you do not see the **Solution Explorer** window, click **Solution Explorer** at the bottom of the **Server Explorer** window to show the **Solution Explorer** again.</span></span>

## <a name="summary"></a><span data-ttu-id="86b8b-488">總結</span><span class="sxs-lookup"><span data-stu-id="86b8b-488">Summary</span></span>

<span data-ttu-id="86b8b-489">在本教學課程中，您已新增訂單和訂單詳細資料架構來追蹤產品的購買。</span><span class="sxs-lookup"><span data-stu-id="86b8b-489">In this tutorial you added order and order detail schemas to track the purchase of products.</span></span> <span data-ttu-id="86b8b-490">您也可以將 PayPal 功能整合到 Wingtip 玩具範例應用程式中。</span><span class="sxs-lookup"><span data-stu-id="86b8b-490">You also integrated PayPal functionality into the Wingtip Toys sample application.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="86b8b-491">其他資源</span><span class="sxs-lookup"><span data-stu-id="86b8b-491">Additional Resources</span></span>

<span data-ttu-id="86b8b-492">[ASP.NET 設定總覽](https://msdn.microsoft.com/library/ms178683(v=vs.100).aspx)</span><span class="sxs-lookup"><span data-stu-id="86b8b-492">[ASP.NET Configuration Overview](https://msdn.microsoft.com/library/ms178683(v=vs.100).aspx)</span></span>  
[<span data-ttu-id="86b8b-493">將具有成員資格、OAuth 和 SQL Database 的安全 ASP.NET Web Forms 應用程式部署至 Azure App Service</span><span class="sxs-lookup"><span data-stu-id="86b8b-493">Deploy a Secure ASP.NET Web Forms App with Membership, OAuth, and SQL Database to Azure App Service</span></span>](https://azure.microsoft.com/documentation/articles/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/)  
[<span data-ttu-id="86b8b-494">Microsoft Azure 免費試用</span><span class="sxs-lookup"><span data-stu-id="86b8b-494">Microsoft Azure - Free Trial</span></span>](https://azure.microsoft.com/pricing/free-trial/)

## <a name="disclaimer"></a><span data-ttu-id="86b8b-495">免責聲明</span><span class="sxs-lookup"><span data-stu-id="86b8b-495">Disclaimer</span></span>

<span data-ttu-id="86b8b-496">本教學課程包含範例程式碼。</span><span class="sxs-lookup"><span data-stu-id="86b8b-496">This tutorial contains sample code.</span></span> <span data-ttu-id="86b8b-497">這類範例程式碼是以「原樣」提供，不含任何種類的擔保。</span><span class="sxs-lookup"><span data-stu-id="86b8b-497">Such sample code is provided "as is" without warranty of any kind.</span></span> <span data-ttu-id="86b8b-498">因此，Microsoft 不保證範例程式碼的精確度、完整性或品質。</span><span class="sxs-lookup"><span data-stu-id="86b8b-498">Accordingly, Microsoft does not guarantee the accuracy, integrity, or quality of the sample code.</span></span> <span data-ttu-id="86b8b-499">您同意以自己的風險使用範例程式碼。</span><span class="sxs-lookup"><span data-stu-id="86b8b-499">You agree to use the sample code at your own risk.</span></span> <span data-ttu-id="86b8b-500">在任何情況下，Microsoft 不會針對任何範例程式碼、內容（包括但不限於任何範例程式碼、內容，或任何因使用任何範例程式碼而產生的任何類型遺失或損毀），以任何方式對您造成任何問題。</span><span class="sxs-lookup"><span data-stu-id="86b8b-500">Under no circumstances will Microsoft be liable to you in any way for any sample code, content, including but not limited to, any errors or omissions in any sample code, content, or any loss or damage of any kind incurred as a result of the use of any sample code.</span></span> <span data-ttu-id="86b8b-501">您將會收到通知，並據此同意補償、儲存及保存 Microsoft 免于任何遺失的損害、遺失的索賠、傷害或任何種類的損害，包括或因您張貼的資料而產生的 occasioned、傳輸、使用或依賴包含（但不限於）其中所表示的觀點。</span><span class="sxs-lookup"><span data-stu-id="86b8b-501">You are hereby notified and do hereby agree to indemnify, save and hold Microsoft harmless from and against any and all loss, claims of loss, injury or damage of any kind including, without limitation, those occasioned by or arising from material that you post, transmit, use or rely on including, but not limited to, the views expressed therein.</span></span>

> [!div class="step-by-step"]
> <span data-ttu-id="86b8b-502">[上一頁](shopping-cart.md)
> [下一頁](membership-and-administration.md)</span><span class="sxs-lookup"><span data-stu-id="86b8b-502">[Previous](shopping-cart.md)
