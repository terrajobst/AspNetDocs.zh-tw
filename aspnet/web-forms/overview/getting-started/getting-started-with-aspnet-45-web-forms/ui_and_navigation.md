...
在 ASP.NET Web Forms 中，主版頁面可讓您為應用程式中的頁面建立一致的版面配置。 單一主版頁面會針對您應用程式中的所有頁面（或頁面群組），定義您想要的外觀和風格和標準行為。 接著，您可以建立包含您想要顯示之內容的個別內容頁面，如上面所述。 當使用者要求內容頁面時，ASP.NET 會將其與主版頁面合併，以產生結合主版頁面版面配置與內容頁面內容的輸出。

新網站需要在每個頁面上顯示單一標誌。 若要新增此標誌，您可以在主版頁面上修改 HTML。

1. 在**方案總管**中，尋找並開啟 [**網站**] 頁面。
