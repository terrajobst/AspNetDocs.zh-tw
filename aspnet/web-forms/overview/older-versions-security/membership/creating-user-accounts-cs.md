...
若要建立此介面，加入下列宣告式標記`Site.master`主版頁面的左側資料行其中文字 「 待辦事項：功能表放在這裡...」目前位於。
...
`SecurityQuestion`標籤和`SecurityAnswer`文字方塊所顯示的預先定義的安全性問題並收集使用者的回應。 請注意，安全性問題和答案會儲存在使用者的使用者為基礎，因此可允許每個使用者定義自己的安全性問題。 不過，此範例中，我已決定使用通用的安全性問題，也就是：「 您偏愛的顏色為何？ 」
